...
public class ResultResp extends BaseActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String villeString = intent.getStringExtra("ville");

        chosenVille = (TextView)findViewById(R.id.textChosenVille);
        chosenVille.setText(villeString);
        ...
        String chosenVilleString = chosenVille.getText().toString();
        int traitPos = chosenVilleString.indexOf(" - ");
        String ville = chosenVilleString.substring(0, traitPos);
        ...
        if(!tableau.contains("accueil")) {
            ...
            showProgress(true);
            String url = "http://172.24.1.1/darties1/private_html/index.php/getTableauAccueilDR?id=" + id + "&id_profil=" + id_profil + "&ville=" + ville + "&token=" + token;
            ...
            TurnoverProductAdapter adapter = new TurnoverProductAdapter(this, products);
            mListView.setAdapter(adapter);

            //Création de la salesView
            salesView = (ListView) findViewById(R.id.salesView);
            SalesProductAdapter salesAdapter = new SalesProductAdapter(this, products);
            salesView.setAdapter(salesAdapter);

            //Création de la marginView
            marginView = (ListView) findViewById(R.id.marginView);
            MarginProductAdapter marginAdapter = new MarginProductAdapter(this, products);
    ...
    private class HttpAsyncTask extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
            ...
            //Création de la turnoverView
            mListView = (ListView) findViewById(R.id.turnoverView);
            TurnoverProductAdapter adapter = new TurnoverProductAdapter(ResultResp.this, products);
            mListView.setAdapter(adapter);

            //Création de la salesView
            salesView = (ListView) findViewById(R.id.salesView);
            SalesProductAdapter salesAdapter = new SalesProductAdapter(ResultResp.this, products);
            salesView.setAdapter(salesAdapter);

            //Création de la marginView
            marginView = (ListView) findViewById(R.id.marginView);
            MarginProductAdapter marginAdapter = new MarginProductAdapter(ResultResp.this, products);
            marginView.setAdapter(marginAdapter);
